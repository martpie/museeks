use std::path::PathBuf;

use geekorm::prelude::TablePrimaryKey;
use geekorm::{GeekConnector, QueryBuilderTrait, Table, TableBuilder};
use lofty::file::{AudioFile, TaggedFileExt};
use lofty::tag::{Accessor, ItemKey};
use log::{error, warn};
use serde::{Deserialize, Serialize};
use ts_rs::TS;
use uuid::Uuid;

/**
 * Track
 * represent a single track, id and path should be unique
 */
#[derive(Debug, Clone, Serialize, Deserialize, Table, TS)]
#[ts(export, export_to = "../../src/generated/typings/index.ts")]
pub struct Track {
    #[geekorm(primary_key)]
    pub id: String,
    #[geekorm(unique)]
    pub path: String, // TODO must be unique, ideally, a PathBuf
    pub title: String,
    pub album: String,
    pub artists: Vec<String>,
    pub genres: Vec<String>,
    pub year: Option<i32>,
    pub duration: i32,
    pub track_no: Option<i32>,
    pub track_of: Option<i32>,
    pub disk_no: Option<i32>,
    pub disk_of: Option<i32>,
}

/**
 * Generate a Track struct from a Path, or nothing if it is not a valid audio
 * file
 */
pub fn get_track_from_file(path: &PathBuf) -> Option<Track> {
    match lofty::read_from_path(&path) {
        Ok(tagged_file) => {
            let tag = tagged_file.primary_tag()?;

            // IMPROVE ME: Is there a more idiomatic way of doing the following?
            let mut artists: Vec<String> = tag
                .get_strings(&ItemKey::TrackArtist)
                .map(ToString::to_string)
                .collect();

            if artists.is_empty() {
                artists = tag
                    .get_strings(&ItemKey::AlbumArtist)
                    .map(ToString::to_string)
                    .collect();
            }

            if artists.is_empty() {
                artists = vec!["Unknown Artist".into()];
            }

            Some(Track {
                // TODO: shouldn't the ID be generated by the ORM?
                id: get_track_id_for_path(&path).unwrap(),
                path: path.to_string_lossy().into_owned(),
                title: tag
                    .get_string(&ItemKey::TrackTitle)
                    .unwrap_or("Unknown")
                    .to_string(),
                album: tag
                    .get_string(&ItemKey::AlbumTitle)
                    .unwrap_or("Unknown")
                    .to_string(),
                artists,
                genres: tag
                    .get_strings(&ItemKey::Genre)
                    .map(ToString::to_string)
                    .collect(),
                year: u32_to_maybe_i32(tag.year()),
                duration: i32::try_from(tagged_file.properties().duration().as_secs()).unwrap_or(0),
                track_no: u32_to_maybe_i32(tag.track()),
                track_of: u32_to_maybe_i32(tag.track_total()),
                disk_no: u32_to_maybe_i32(tag.disk()),
                disk_of: u32_to_maybe_i32(tag.disk_total()),
            })
        }
        Err(err) => {
            warn!("Failed to get ID3 tags: \"{}\". File {:?}", err, path);
            None
        }
    }
}

fn u32_to_maybe_i32(value: Option<u32>) -> Option<i32> {
    value.and_then(|v| {
        if v <= i32::MAX as u32 {
            i32::try_from(v).ok()
        } else {
            None
        }
    })
}

/**
 * Generate an ID for a track based on its location.
 *
 * We leverage UUID v3 on tracks paths to easily retrieve tracks by path.
 * This is not great and ideally we should use a DB view instead. One day.
 */
pub fn get_track_id_for_path(path: &PathBuf) -> Option<String> {
    match std::fs::canonicalize(path) {
        Ok(canonicalized_path) => {
            return Some(
                Uuid::new_v3(
                    &Uuid::NAMESPACE_OID,
                    canonicalized_path.to_string_lossy().as_bytes(),
                )
                .to_string(),
            );
        }
        Err(err) => {
            error!(r#"ID could not be generated for path {:?}: {}"#, path, err);
            return None;
        }
    };
}
