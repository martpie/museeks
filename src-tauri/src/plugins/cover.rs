...
use crate::libs::utils::{scan_dir, TimeLogger};
...
#[memoize::memoize]
fn get_cover_from_filesystem(dir_path: PathBuf) -> Option<String> {
...
#[tauri::command]
async fn get_cover(path: String) -> AnyResult<Option<String>> {
    // 1. Try to get the Cover from the ID3 tag
    let time = TimeLogger::new("get_cover".into());
    let result = match get_cover_from_id3(path.clone()) {
        Some(path) => Ok(Some(path)),
        // 2. Cover was not found, so let's fallback to scanning the directory
        // for a valid cover file
        None => Ok(get_cover_from_filesystem(
