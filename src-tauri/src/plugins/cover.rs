...
fn get_cover_from_id3(path: &str) -> Option<String> {
...
fn get_cover_from_filesystem(path: &str) -> Option<String> {
    let track_path = PathBuf::from_str(path).unwrap();
    let dir_path = track_path.parent().unwrap().to_path_buf();
