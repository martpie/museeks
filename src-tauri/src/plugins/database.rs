...
impl DB {
    ...
    pub async fn get_all_tracks(&self) -> AnyResult<Vec<Track>> {
        ...
        let tracks = self.decode_tracks(docs);
    ...
    pub async fn get_tracks(&self, track_ids: Vec<String>) -> AnyResult<Vec<Track>> {
        ...
        self.decode_tracks(docs)
    }

    /**
     * Decode the content for a given set of track document
     */
    fn decode_tracks(&self, docs: Vec<OwnedDocument>) -> AnyResult<Vec<Track>> {
        let mut tracks = vec![];

        for doc in docs {
            let deserialized = Track::document_contents(&doc)?;
            tracks.push(deserialized);
        }
        ...
        .map(|path| -> Option<Track> {
            ...
            match lofty::read_from_path(&path) {
                Ok(tagged_file) => {
                    ...
                    Some(Track {
                        _id: Uuid::new_v3(&Uuid::NAMESPACE_OID, path.as_bytes()).to_string(),
...
async fn reset(db: State<'_, DB>) -> AnyResult<()> {
    ...
    // Reset tracks
    let track_collection = db.tracks.collection::<Track>();
    let track_docs = track_collection.all().await?;

    for doc in track_docs {
        track_collection.delete(&doc).await?;
    }

    // Reset playlists
    let playlists_collection = db.playlists.collection::<Track>();
    let playlists_docs = playlists_collection.all().await?;

    for doc in playlists_docs {
        playlists_collection.delete(&doc).await?;
    }
