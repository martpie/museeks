...
import { Config, PlaylistDoc, TrackDoc } from '../../generated/typings';
...
export default function TracksList(props: Props) {
  ...
  const onEnter = useCallback(
    async (index: number, tracks: TrackDoc[]) => {
      if (index !== -1) playerAPI.start(tracks, tracks[index].id);
    },
    [playerAPI],
  );

  const onControlAll = useCallback((tracks: TrackDoc[]) => {
    setSelected(tracks.map((track) => track.id));
  }, []);

  const onUp = useCallback(
    (index: number, tracks: TrackDoc[], shiftKeyPressed: boolean) => {
  ...
  const showContextMenu = useCallback(
    (_e: React.MouseEvent, index: number) => {
      ...
      //   },
      // ];
      // for (const artist of track.doc.artists) {
      //   template.push({
      ...
      // }
      // template.push({
      //   label: `Search for "${track.doc.album}"`,
      //   click: () => {
      //     libraryAPI.search(track.doc.album);
      //   },
      // });
          ...
          {virtualizer.getVirtualItems().map((virtualItem) => {
            ...
            return (
              <TrackRow
                ...
                reordered={(reordered && reordered.includes(track.id)) || false}
